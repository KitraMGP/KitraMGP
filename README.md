...
日常工作/娱乐使用的操作系统：Arch Linux，使用 KDE+Wayland
...
学业上，最近在做工业大数据方面的科研性项目，涉及到高性能系统的架构设计和时序数据库，以及深度学习科研。

我在进一步学习 Web 技术，并在学习更多的嵌入式技术。

后端我常用 Spring Boot，但现在正在转向 Golang；前端我会使用 Vue3、Svelte，使用 TypeScript 编程，还使用 Tailwind CSS。

我使用 Podman 构建开发所需的环境，还使用 Podman Compose。

我对嵌入式开发很感兴趣，希望使用 ESP32 或 CH32Vxxx 开发有趣的产品（例如音乐制作方面的小设备），我打算在大四进行这方面的实践！

我计划对我的静态博客进行升级，为其加入 Web 端博客编辑器，我打算在大四开展这些工作。

我正在探索桌面客户端开发的新技术（iced）和简单的游戏渲染应用（OpenGL/bgfx）。iced 是基于 Rust 的跨平台 UI 框架，可零配置直接交叉编译，可以实现美观界面并有着相对低的内存占用；bgfx 是一个跨平台渲染引擎，为 Vulkan 抽象出类似 OpenGL 的高级 API，并支持 Wayland。
